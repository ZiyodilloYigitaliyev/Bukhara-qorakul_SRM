...
async def get_week_schedule(class_name: str, student_id: int, db: AsyncSession) -> list[dict]:
    """Hafta bo'yicha: har kun uchun items ro'yxati qaytaradi."""
    out: list[dict] = []
    for day in DAYS:
        stmt = (
            select(Schedule)
            ...
            .where(or_(Schedule.class_name == class_name, Schedule.student_id == student_id))
...
def _today_name() -> str:
    # Serverda TZ=Asia/Tashkent qo‘yilgan — bevosita bugungi kun nomi
    return date.today().strftime("%A")  # "Monday" ...

async def get_today_schedule(class_name: str, student_id: int, db: AsyncSession) -> list[ScheduleOut]:
    today = _today_name()
    stmt = (
        select(Schedule)
        .options(joinedload(Schedule.teacher))
        ...
        .where(
            or_(
                Schedule.class_name == class_name,  # guruh darsi
                Schedule.student_id == student_id   # individual dars
            )
        )
        .order_by(Schedule.start_time)
    )
